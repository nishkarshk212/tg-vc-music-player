...
async def pin(_, message: Message):
    ...
    if not await has_permission(
        message.from_user.id, message.chat.id, "can_pin_messages"
    ):
        return await message.reply_text(
            "**ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴘᴇʀᴍɪssɪᴏɴ ᴛᴏ ᴘɪɴ ᴍᴇssᴀɢᴇs.**"
        )
                ...
                [
                    [
                        InlineKeyboardButton(
                            "📝 ᴠɪᴇᴡ ᴍᴇssᴀɢᴇ", url=message.reply_to_message.link
                        )
                    ]
                ]
            ),
...
async def set_photo(_, message: Message):
    ...
    if not await has_permission(
        message.from_user.id, message.chat.id, "can_change_info"
    ):
    ...
    try:
        ...
        await message.reply_text(
            f"**ɢʀᴏᴜᴘ ᴘʜᴏᴛᴏ ᴜᴘᴅᴀᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ!**\nʙʏ {message.from_user.mention}"
        )
    except Exception as e:
