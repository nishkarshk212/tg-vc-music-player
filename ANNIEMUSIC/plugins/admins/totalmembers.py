        ...
        if len(members_list) % 100 == 0:
            try:
                await processing_msg.edit_text(
                    f"Collected {len(members_list)} members so far..."
                )
            except Exception:
                pass
    return members_list
...
async def user_command(_, message):
    ...
    await message.reply_text(
        "In which format do you want the members list?", reply_markup=keyboard
    )
...
async def members_format_callback(_, callback_query):
    ...
    if format_choice == "csv":
        ...
        text_lines = [
            f"{member['username']} - {member['userid']}" for member in members_list
        ]
        txt_str = "\n".join(text_lines)
    ...
    await app.send_document(
        chat_id, document=file_bytes, caption=caption_text, file_name=file_name
    )
