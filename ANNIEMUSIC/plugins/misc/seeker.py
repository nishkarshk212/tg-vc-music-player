...
async def get_movie_info(query: str) -> str:
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        details = await client.get(
            f"{TMDB_BASE}/movie/{movie_id}", params={"api_key": TMDB_API_KEY}
        )
        details_data = details.json()

        cast = await client.get(
            f"{TMDB_BASE}/movie/{movie_id}/credits", params={"api_key": TMDB_API_KEY}
        )
        cast_data = cast.json()
        actors = (
            ", ".join([actor["name"] for actor in cast_data.get("cast", [])[:5]])
            or "N/A"
        )
