...
async def remove_media(_, message: Message):
    ...
    if len(message.command) < 2:
        return await message.reply_text(
            "ℹ️ Use `/remove audio` or `/remove video`.", quote=True
        )
    ...
    try:
        ...
        if command == "audio":
            ...
            await app.send_audio(
                message.chat.id, output_audio, caption="🎧 Audio extracted."
            )
            ...
            await app.send_video(
                message.chat.id, output_video, caption="🎞️ Video with no audio."
            )
